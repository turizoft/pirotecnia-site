      ...
      <section className='bg-white py-20 md:py-28'>
        <div className='container mx-auto px-4'>
          <div className='mb-12 grid gap-8 md:grid-cols-2'>
            {(locations.length ? locations : Array.from({ length: 4 })).map((location: any, idx) => (
              <div
                key={location?.id || idx}
              ...
              >
                ...
                <div className='relative z-10 space-y-4'>
                  ...
                  <div className='flex items-center gap-3'>
                    ...
                    <p className='font-semibold text-black'>
                      {location?.phone ?? 'Teléfono por confirmar'}
                    </p>
                  </div>
                  <div className='flex items-start gap-3'>
                    <Clock className='mt-1 h-5 w-5 flex-shrink-0 text-black' />
                    <p className='text-sm font-semibold text-black'>
                      {location?.hours ?? 'Horarios disponibles bajo solicitud'}
                    </p>
                  </div>
                </div>
                <Link
