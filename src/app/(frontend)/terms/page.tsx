...
export default async function TermsPage() {
  ...
  const terms = await payload.findGlobal({ slug: 'terms' }).catch(() => null);

  const hero = (terms as any)?.hero ?? {
    title: 'Términos de',
    titleAccent: 'Servicio',
    subtitle: 'Nuestros compromisos y las reglas de uso',
  };

  const content = (terms as any)?.content;
          ...
          <div className='container relative z-10 mx-auto px-4'>
            {content ? (
              ...
              <FadeIn className='mx-auto max-w-md rounded-2xl border border-white/5 bg-white/[0.02] p-12 text-center backdrop-blur-sm'>
                <Scale className='mx-auto mb-6 h-16 w-16 text-white/20' />
